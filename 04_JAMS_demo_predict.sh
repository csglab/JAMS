...
DATA_DIR=${DATA}/04_predict/data_search
PRED_BIND=${DATA}/04_predict/CTCF_search_motif
...
./JAMS --task PREDICT \
       ...
       --regions ${REGION} \
       --data_dir ${DATA_DIR} \
       --predicted_binding_prefix ${PRED_BIND} \
       --wgbs_met_data ${METH} --wgbs_unmet_data ${UNMETH} \
       --dna_acc_map ${DNA_ACC} --genome_fa ${GEN_FA} \
       --chr_sizes ${CHR_SIZES} --pfm ${PFM}


DATA_DIR=${DATA}/04_predict/data_start
PRED_BIND=${DATA}/04_predict/CTCF_use_start_as_motif
